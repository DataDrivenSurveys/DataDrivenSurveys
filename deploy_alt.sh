...
docker compose --env-file .env.deploy.local -f compose.deploy_alt.yml build || exit 1
...
cd dds
if [ -f compose.deploy_alt.yml ]; then
  sudo docker compose -f compose.deploy_alt.yml down
fi
EOF
# cd /home/admin
# mv dds/volumes ./
...
scp -o BatchMode=yes -B -i "${SERVER_SSH_KEY}" -p compose.deploy_alt.yml .env.deploy.local "${docker_exported_images[@]}" "${ssh_address}":dds/
...
sudo bash -c "docker compose --env-file .env.deploy.local -f compose.deploy_alt.yml up -d"
