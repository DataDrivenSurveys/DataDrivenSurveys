...
function InterpolateContent({i18nKey}: InterpolateContentProps): JSX.Element {
  return (
    <Trans
      i18nKey={i18nKey}
          ...
          {Object.keys(formattedSections[parentSection]).length > 0 && (
            Object.keys(formattedSections[parentSection]).map((sectionName, indexSection) => (
              <React.Fragment key={indexSection}>
                <Typography paragraph>
                  <FormatSubSection title={`${indexSection + 1}. ` + t(`${parentSection}.${sectionName}.title`)} content={t(`${parentSection}.${sectionName}.content`)}/>
                      ...
                      <Typography paragraph sx={{paddingLeft: "8px"}}>
                        {formattedSections[parentSection][sectionName].map((subsection, indexSubsection) => (
                          <Typography key={indexSubsection}>
                            <FormatSubSection title={`${indexSection + 1}.${indexSubsection + 1}. ` + t(`${parentSection}.${sectionName}.${subsection}.title`)} content={t(`${parentSection}.${sectionName}.${subsection}.content`)}/>
                          </Typography>
                        ))}
                      </Typography>
                    </>
    ...
    <Container sx={{width: "80%"}}>
      ...
      <Typography paragraph>
        <Trans
          ...
          components={{
            email:
              <Link href={`mailto:${t('privacy_policy.contact_information.contact_email')}`} rel="noopener noreferrer" color="primary"/>
          }}
