...
const PageParticipantConnection = ({ placeholder = false }: PageParticipantConnectionProps): JSX.Element => {
  ...
  const fetchOauthDataProviders = useCallback(async () => {
    ...
    response.on('2xx', async (status: number, data_providers: API.Respondent.DataProvider[]): Promise<void> => {
      const tokens: Models.Dist.RespondentTempTokens[] = JSON.parse(localStorage.getItem('RespondentTempTokens') || '[]');
      if (status === 200) {
        const providersPromises = data_providers.map(async (data_provider: API.Respondent.DataProvider): Promise<Models.Dist.DataProvider> => {
  ...
  const handleConnect = useCallback(async (data_provider_name: string) => {
  ...
  const handleDisconnect = useCallback(async (data_provider_name: string) => {
