...
def test_get_redirect_uri(provider_name, monkeypatch):
    ...
    if issubclass(provider_class, OAuthDataProvider):
        assert (
            provider_class.get_redirect_uri() == "http://testurl.com/dist/redirect/" + provider_name.lower()
...
PARAMETERIZED_PROVIDERS = [
    (provider, EXPECTED_RESULTS[provider]) for provider in REGISTERED_DATAPROVIDERS if provider in EXPECTED_RESULTS
]
...
def test_get_all_form_fields():
    ...
    for provider_form in data_provider_data:
        ...
        assert (
            provider_form["instructions"] == expected_instruction_pattern
...
def test_builtin_variables(provider_name):
    ...
    for variable in builtin_variables:
        ...
        assert variable["qualified_name"] == builtin_variable_to_qualname(
            variable
        ), "The qualified name should be the data provider name concatenated with the variable name."
        assert (
            variable["data_provider"] == provider_name.lower()
...
def test_data_extraction_builtin_variables(
    ...
    custom_variables_attributes_count = sum([len(cv["cv_attributes"] * 2) for cv in custom_variables]) + len(
        custom_variables
    )

    assert (
        len(builtin_data_to_upload) == len(builtin_variables) * 2
    ), f"Check the total number of builtin variables to upload for {data_provider_name}, including .exists."
    assert (
        len(custom_data_to_upload) == custom_variables_attributes_count
...
def test_get_used_variables(provider_name):
    ...
    used_variables = DataProvider.get_used_variables(
        project_builtin_variables=project_buitin_variables, project_custom_variables=project_custom_variables
    )
    ...
    for used_variable in used_variables:
        ...
        assert (
            variable["data_origin"] and len(variable["data_origin"]) > 0
