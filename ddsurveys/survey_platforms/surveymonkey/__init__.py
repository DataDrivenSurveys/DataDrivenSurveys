...
from ..bases import OAuthSurveyPlatform, FormField, FormButton
...
class SurveyMonkeySurveyPlatform(OAuthSurveyPlatform):
    ...
    def __init__(self, survey_id: str = None, client_id: str = None, client_secret: str = None, access_token: str = None, **kwargs):
        """
    ...
    def init_api_client(self, client_id: str, client_secret: str,
                        redirect_uri: str = None, access_token: str = None, *args, **kwargs) -> None:
        if redirect_uri is None:
            redirect_uri = self.get_redirect_uri()
        self.api_client = SMClient(client_id=client_id, client_secret=client_secret, redirect_uri=redirect_uri,
    ...
    def handle_project_creation(self, project_name: str,
                                use_existing_survey: bool = False) -> Tuple[int, str, str, str, Dict[str, Any]]:
        ...
        if use_existing_survey:
            ...
            try:
                ...
                return 200, "api.survey_platforms.project_creation_success", "Project created successfully", survey_name, survey_platform_fields
            except Exception as e:
                return (400, "api.survey.unknown_error_occurred",
                        "Unknown error occurred, please check your App Credentials", None, {})
        else:
            try:
    ...
    def handle_variable_sync(self, enabled_variables: dict) -> Tuple[int, str, str]:
        ...
        variables : dict[str, str] = {variable['qualified_name']: variable['test_value_placeholder'] for variable in enabled_variables}

        modified_variables = {key.replace('.', '_').replace('[', '_').replace(']', ''): value for key, value in variables.items()}
    ...
    def get_specific_survey(self, survey_id: str) -> dict:
        ...
        logger.debug(f"SurveyMonkeySurveyPlatform.get_specific_survey: {self.access_token}")
    ...
    def modify_specific_survey(self, survey_id: str, custom_variables: dict[str, str]) -> dict:
        ...
        endpoint = "/surveys/{}".format(survey_id)
        url = SurveyMonkeySurveyPlatform.API_URL + endpoint
        _headers = {"Authorization": "Bearer %s" % self.access_token, "Content-Type": "application/json"}
