...
class SpotifyDataProvider(OAuthDataProvider):
    ...
    _scopes: ClassVar[tuple[str, ...]] = (
        ...
        "user-read-private",  # 6 Read user's profile info (no PII - just country, display name, subscription level, public profile url)
    )
    ...
    _categories_scopes: ClassVar[dict[str, str]] = {
        "Playlist": f"{_scopes[0]}, {_scopes[1]}",
        "Account": f"{_scopes[2]}, {_scopes[6]}",
        "Devices": _scopes[2],
        "Episode": _scopes[3],
        "Artist": f"{_scopes[0]}, {_scopes[1]}",
        "Shows": _scopes[3],
        "Genres": f"{_scopes[0]}, {_scopes[1]}",
    }
    ...
    def init_api_client(
        ...
        self.api_client = spotipy.Spotify(
            auth=self.access_token, oauth_manager=self.oauth_client
        )  # to get the playlists and so on, we only need to use an access_token
        if refresh_token:
            self.oauth_client.refresh_access_token(refresh_token)

    def init_oauth_client(self, *args, **kwargs) -> None:
        required_scopes: list[str] = self.get_required_scopes(
            self.builtin_variables, self.custom_variables
        )
        ...
        self.oauth_client = SpotifyOAuth(
            ...
            scope=kwargs.get("required_scopes"),
            cache_handler=None,
            cache_path=None,
        )
    ...
    def get_authorize_url(
        ...
        # create instance of SpotifyOAuth with the required scopes
        self.oauth_client.scope = normalize_scope(
            self.get_required_scopes(builtin_variables, custom_variables)
        )
    ...
    def request_token(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.oauth_client.refresh_access_token(refresh_token)
            user_info = {"username": user["display_name"], "user_id": user["id"]}
    ...
    def top_artist(self):
        ...
        for p in playlists["items"]:
            playlist_items.append(
                self.api_client.playlist_items(p["id"], additional_types=("track"))
            )
    ...
    def genres(self, idx: int):  # idx is one or two
        ...
        # iterate over every playlist p
        for p in playlists["items"]:
            tracks_in_playlist = self.api_client.playlist_items(
                p["id"], additional_types=("track")
            )["items"]
            for track in tracks_in_playlist:
                artists = track["track"]["album"]["artists"]
                for artist in artists:
