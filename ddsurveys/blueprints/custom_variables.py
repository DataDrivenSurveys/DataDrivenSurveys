...
from ..models import Project, Researcher, Collaboration, get_db
...
def is_valid_variable_name(variable_name: str) -> tuple[bool, str, str]:
    ...
    if not variable_name:
        return False, "api.custom_variables.error.name_required", "Variable name is required."

    if not variable_name[0].isalpha() and not variable_name[0] == "_":
        return False, "api.custom_variables.error.must_start_with_letter_or_underscore", "Variable names must start with a letter or underscore."

    if not all(char.isalnum() or char == "_" for char in variable_name):
        return False, "api.custom_variables.error.only_alphanumeric_and_underscore", "Variable names can only contain alpha-numeric characters and underscores."

    if iskeyword(variable_name):
        return False, "api.custom_variables.error.cannot_be_python_keyword", "Variable name cannot be a Python keyword."
...
def check_custom_variable_data(data):
    ...
    if not data.get("data_provider"):
        return False, "api.custom_variables.error.data_provider_required", "Data provider is required."

    # Check if the data category is set
    if not data.get("data_category"):
        return False, "api.custom_variables.error.data_category_required", "Data category is required."

    # Check if the attributes are set
    if not data.get("cv_attributes"):
        return False, "api.custom_variables.error.attributes_required", "Attributes are required."

    # Check if the selection is set
    if not data.get("selection"):
        return False, "api.custom_variables.error.selection_required", "Selection is required."

    if data.get("selection"):
        selection = data.get("selection")
        if not selection.get("operator"):
            return False, "api.custom_variables.error.selection_operator_required", "Selection operator is required."

        if selection.get("operator") != "random" and not selection.get("attr"):
            return False, "api.custom_variables.error.selection_attr_required", "Selection attribute is required."
    ...
    if data.get("filters"):
        ...
        for filter in filters:
            if not filter.get("attr"):
                return False, "api.custom_variables.error.filter_attr_required", "Filter attribute is required."
            if not filter.get("operator"):
                return False, "api.custom_variables.error.filter_operator_required", "Filter operator is required."
            if not filter.get("value"):
                return False, "api.custom_variables.error.filter_value_required", "Filter value is required."
...
def add_custom_variable_to_project():
    ...
    with get_db() as db:
        ...
        if variable_name in existing_variable_names:
            ...
            return jsonify({"message": {"id": "api.projects.custom_variables.variable_exists", "text": f"Variable name '{variable_name}' already exists. Use a different name."}}), 400

        try:
            current_custom_variables.append(data)
            project.custom_variables = current_custom_variables
            flag_modified(project, "custom_variables") # This is required to notify SQLAlchemy that the custom_variables field has changed
            db.commit()
            ...
            return jsonify({"message": {"id": "api.projects.custom_variables.failed_to_add", "text": "Failed to add custom variable"}}), 500
...
def get_project_custom_variable(variable_id):
    with get_db() as db:
        ...
        variable = next((var for var in project.custom_variables if var.get("id", -1) == variable_id), None)
        if not variable:
            return jsonify({"message": {"id": "api.projects.custom_variables.not_found", "text": "Custom variable not found"}}), 404
...
def update_project_custom_variable(variable_id):
    with get_db() as db:
        ...
        if not is_valid:
            logger.error(f"Failed to update custom variable: {message_id}: {validation_msg}")
            return jsonify({"message": {"id": message_id, "text": validation_msg}}), 400
