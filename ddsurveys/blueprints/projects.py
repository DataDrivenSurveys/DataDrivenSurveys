...
from ..models import Project, Researcher, Collaboration, DataConnection, Respondent, get_db
...
def list_projects():
    ...
    with get_db() as db:
        ...
        # get the projects
        projects = db.query(Project).join(Collaboration).filter(
...
def create_project():
    ...
    with get_db() as db:
        ...
        are_fields_valid, missing_field_key = platform_class.check_input_fields(fields, override_required, SurveyPlatform)
        ...
        status, message_id, text_message, project_name, survey_platform_fields = survey_platform.handle_project_creation(project_name, use_existing_survey)
...
def delete_project(id):
    ...
    with get_db() as db:
        ...
        collaboration = db.query(Collaboration).filter_by(project_id=id, researcher_id=researcher.id).first()
...
def delete_respondents(id):
    ...
    with get_db() as db:
        ...
        return jsonify({"message": {"id": "api.projects.respondents.deleted_successfully", "text": "All project respondents deleted"}}), 200
