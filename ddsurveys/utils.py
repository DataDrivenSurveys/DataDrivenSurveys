...
if TYPE_CHECKING:
    from ddsurveys.get_logger import ProjectLoggerLevels


# TODO: switch to using a json file for the config


class EnvDict(TypedDict):
    """Environment variables used by the DDS backend.

    These variables need to be defined in .env.local files inside of the
    ddsurveys directory or using shell environment variables.
    """

    DATABASE_URL: str | None
    DDS_ENV: Literal["development", "production", "testing", ""] | None
    DDS_LOG_LEVEL: ProjectLoggerLevels | None
    DDS_ONLY_LOG_DDS: Literal["true", "false", ""] | None
    DDS_SET_LEVEL_FOR_ALL_LOGGERS: Literal["true", "false", ""] | None
    FLASK_ENV: Literal["development", "production", "testing", ""] | None
    FRONTEND_URL: str | None
    JWT_SECRET_KEY: str | None


def get_and_load_env(path: Path | str, *, in_module_root: bool = False) -> EnvDict:
    ...
    Args:
        path: A Path object or string specifying the path to the `.env` file.
            This can be an absolute path or, if `in_module_root` is True,
            a path relative to the module's root directory.
    ...
    env: EnvDict = cast(EnvDict, dotenv_values(str(path)))  # pyright: ignore[reportInvalidCast]
    if path_dot_local.is_file():
        env = cast(EnvDict, cast(object, {**env, **dotenv_values(str(path_dot_local))}))

    # Load .env file variables into the environment variables
    for k, v in env.items():
        if v is not None:
            os.environ[k] = v  # pyright: ignore[reportArgumentType]
    return env


def handle_env_file(path: str | None = None) -> EnvDict:
